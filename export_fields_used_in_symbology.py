...
print("\n--- Symbology and Reference Scale Information ---\n")
print(f"{'Map Name':<20} {'Layer Name':<30} {'Symbology Field(s)':<40} {'Reference Scale':<15}")
    ...
    for lyr in m.listLayers():
        if lyr.isFeatureLayer:
            ...
            if hasattr(sym, "renderer"):
                ...
                data.append([m.name, lyr.name, symbology_field, ref_scale])
                
                # Print layer details to console
                print(f"{m.name:<20} {lyr.name:<30} {str(symbology_field):<40} {ref_scale:<15}")

# Create a DataFrame and export to Excel
df = pd.DataFrame(data, columns=["Map Name", "Layer Name", "Symbology Field(s)", "Reference Scale"])
